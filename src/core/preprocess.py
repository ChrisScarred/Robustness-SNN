...
def extract_mfscs(
    ...
    framed_audio = get_striding_windows(audio.content, stride_samples, audio.content.size, frame_samples, stride_samples)
    # pad each window
    framed_audio = np.apply_along_axis(lambda x: np.pad(x, (pad_samples,)), 1, framed_audio)

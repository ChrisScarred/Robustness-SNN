...
class Config:
    ...
    def get_prep_vars(self) -> Tuple[int, int, int, int]:
        n_frames = self.get("model_params.time_frames")
        overlap = self.get("model_params.dtf.overlap")
        pad = self.get("model_params.dtf.pad")
        freq_bands = self.get("model_params.mfsc.freq_bands")
